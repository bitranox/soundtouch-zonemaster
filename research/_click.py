...
This is a deliberate copy of ``tools/_click.py`` and
``src/soundtouch_zonemaster/adapters/cli/typed_click.py``. The research scripts are standalone files
that must run where the package is not installed, so they cannot import it - the same reason
``capture_model.py`` duplicates a few members of ``src/soundtouch_zonemaster/domain/enums.py``.
